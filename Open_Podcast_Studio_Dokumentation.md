...
- **`server.py`**: FastAPI/Uvicorn application
  ...
  - Chunk upload API
  - Finish/merge pipeline for WAV and optional MP4 output
  - Host and Admin APIs
...
Each guest recording belongs to a session. Session IDs are internal, path-safe identifiers.
...
### 7.5 Microphone control

The Recorder reports its available microphones and current device. The Host can request a device change for a specific guest:
...
The Host/Admin UI exposes:

- individual WAV downloads for each guest track
- inline WAV previews where supported
- ZIP export of all finished WAV tracks for a room
